...
class TileRenderer {
    renderTiles(ctx, tileAtlas, mapLayout, camera) {
...
function isWithinRenderView(tilePos, camera, renderWidth, renderHeight) {
    const camCenter = camera.getCenter();
    return (tilePos.x > camCenter.x - renderWidth / 2 &&
